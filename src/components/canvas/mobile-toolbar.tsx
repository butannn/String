  ...
  return (
    <>
      ...
      {(selectedId || selectedAttachmentId) && !mobileSheetOpen ? (
        <div
          className="fixed inset-x-3 z-50 rounded-xl border border-zinc-200 bg-white/95 p-2 shadow-xl backdrop-blur md:hidden"
      ...
      {/* Fixed bottom bar */}
      {!mobileSheetOpen ? (
        <div className="fixed inset-x-0 bottom-0 z-40 border-t border-zinc-200 bg-white/95 px-3 pb-[max(12px,env(safe-area-inset-bottom))] pt-2 backdrop-blur dark:border-zinc-800 dark:bg-zinc-900/95 md:hidden">
          <div className="grid grid-cols-3 gap-2">
            <Button
              className="h-11"
              variant="outline"
